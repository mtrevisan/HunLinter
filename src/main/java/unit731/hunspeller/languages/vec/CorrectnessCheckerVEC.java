...
if(WordVEC.countAccents(subword) > 0){
//				String morphologicalFields = production.getMorphologicalFields();
//				int stemStartIndex = morphologicalFields.indexOf(MorphologicalTag.TAG_STEM) + MorphologicalTag.TAG_STEM.length();
//				int stemEndIndex = morphologicalFields.indexOf(StringUtils.SPACE, stemStartIndex);
//				if(stemEndIndex < 0)
//					stemEndIndex = morphologicalFields.length();
//				if(stemEndIndex - stemStartIndex > 1){
//					String stem = morphologicalFields.substring(stemStartIndex, stemEndIndex);
//					Hyphenation hyphenation = hyphenator.hyphenate(stem);
//					List<Integer> stressIndexes = orthography.getStressIndexFromLast(hyphenation.getSyllabes());
//					if(!stressIndexes.isEmpty() && stressIndexes.get(0) > 0){
