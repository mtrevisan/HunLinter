...
//		}
//
//		int nroptions = transitions.size() - (state.isAccept()? 0: 1);
//		int option = getRandomInt(random, 0, nroptions);
...
//	}
	...
	private void generate(StringBuilder builder, State state, Random random){
		List<Transition> transitions = state.getSortedTransitions(false);
		if(transitions.isEmpty()){
			assert state.isAccept();
		...
		//moving on to next transition
		Transition transition = transitions.get(option - (state.isAccept()? 1: 0));
		appendChoice(random, transition, builder);

		generate(builder, transition.getDest(), random);
	}

	private void appendChoice(Random random, Transition transition, StringBuilder builder){
		char c = (char)getRandomInt(random, transition.getMin(), transition.getMax());
		builder.append(c);
	}
