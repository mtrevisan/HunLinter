...
/**
 * An implementation of a compact Tree.
 * 
...
public class Tree<S, H, V>{
	...
	public V get(S sequence){
		TreeNode<S, H, V> node = searchAndApply(sequence, TrieMatch.EXACT, null);
	...
	public V remove(S sequence){
		TreeNode<S, H, V> node = searchAndApply(sequence, TrieMatch.EXACT, (parent, stem) -> parent.removeChild(stem, sequencer));
		return (node != null? node.getValue(): null);
	}
	 ...
	 */
	private TreeNode<S, H, V> searchAndApply(S sequence, TrieMatch matchType, BiConsumer<TreeNode<S, H, V>, H> callback){
		Objects.requireNonNull(sequence);
